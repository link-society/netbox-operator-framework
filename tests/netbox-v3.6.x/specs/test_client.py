...
async def test_from_api(netbox_token: str):
    ...
    assert client.version.major == 3
    assert client.version.minor == 6
    assert client.version.patch == 9
    assert client.version.meta == ""
    assert str(client.version) == "3.6.9 (3.6)"
...
async def test_from_json_file(netbox_token: str):
    settings = Settings(
        netbox_api="http://localhost:8080",
        netbox_token=netbox_token,
        netbox_schema="./tests/netbox-v3.6.x/openapi.json",
        server_bind_host="0.0.0.0",
        server_bind_port=5000,
        server_callback_name="pytest",
        server_callback_url="http://host.docker.internal:5000",
...
async def test_from_yaml_file(netbox_token: str):
    settings = Settings(
        netbox_api="http://localhost:8080",
        netbox_token=netbox_token,
        netbox_schema="./tests/netbox-v3.6.x/openapi.yml",
        server_bind_host="0.0.0.0",
        server_bind_port=5000,
        server_callback_name="pytest",
        server_callback_url="http://host.docker.internal:5000",
...
async def test_unsupported_schema_file_format(netbox_token: str):
    settings = Settings(
        netbox_api="http://localhost:8080",
        netbox_token=netbox_token,
        netbox_schema="./tests/netbox-v3.6.x/openapi.txt",
        server_bind_host="0.0.0.0",
        server_bind_port=5000,
        server_callback_name="pytest",
        server_callback_url="http://host.docker.internal:5000",
